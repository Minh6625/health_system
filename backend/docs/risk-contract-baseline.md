...
| Field | Value |
| --- | --- |
| Baseline version | `v1.5.2` (Phase 8 slice 4b — mobile SHAP detail screen) |
| Wire version | `0.5.0` (unchanged — mobile parser of existing v1.0 clinician fields) |
| Captured from branch | `feat/risk-core-phase8-shap-detail-screen` |
...
| SHAP detail screen (mobile) | `lib/features/analysis/presentation/screens/risk_shap_detail_screen.dart` (route `/risk-report/shap-detail`) |
...
| `v1.5.2` | 2026-04-27 | `feat/risk-core-phase8-shap-detail-screen` | **Plan's Phase 8 slice 4b — mobile SHAP detail screen (mobile-only, no backend change)**: extended `RiskReportDetailEntity` with optional `shapDetails: ShapWaterfall?` + `modelRequestId: String?` (both null on patient-mode loads, both populated on clinician-mode loads). New `ShapWaterfall` + `ShapContribution` value objects with forward-compat parsing in `_parseShapDetails` (non-map shap_details → null; empty feature names skipped; missing `impact` falls back to `abs(shap_value)`; `available=false` preserved as empty so the screen renders the rule-based-fallback empty state). New `RiskShapDetailScreen` rendering: header with base_value + total contribution + feature count; horizontally-scrolling list of bars sorted by descending `\|impact\|`, capped at top 20 for phone scannability; positive bars in `colorScheme.error`, protective (negative) bars in `colorScheme.tertiary`; signed values with leading `+`/`-` for sign-readability; dedicated row for `model_request_id` with copy-to-clipboard button + snackbar feedback; standard disclaimer card. Server-truth gating: `RiskReportDetailScreen` only renders the "Xem chi tiết SHAP (clinician)" entry-point card when `detail.hasClinicianShapDetails == true` (which is itself only true when the response carried `shap_details.available=true` from the backend's audience=clinician path) — the toggle alone isn't enough; a non-clinician with the toggle on still gets a 403 and falls back to the patient flow, so the entity simply never gets populated. New route `riskShapDetail = '/risk-report/shap-detail'` accepting the entity in route arguments to avoid a re-fetch. 19 new Flutter tests (10 parser + ShapWaterfall maths; 9 widget tests covering: unavailable state on null/available=false, sorted-by-impact rendering, signed-value formatting, model_request_id row visibility, disclaimer presence, take(20) cap via scrollUntilVisible). Backend untouched, wire version unchanged. |
