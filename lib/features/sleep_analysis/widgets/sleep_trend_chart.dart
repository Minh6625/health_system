...
class _SleepTrendChartState extends State<SleepTrendChart> {
              ...
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) => Text(
                  ...
                  style: const TextStyle(
                      color: Color(0xFF5B7FA6), fontSize: 10),
                ),
              ),
            ),
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  ...
                  final isActive = i == _touchedIndex || i == highlightedIndex;
                  // Show "dd/MM" — e.g. "15/03"
                  final label =
                      DateFormat('dd/MM').format(slots[i].date);
                  return Padding(
  ...
  List<_DaySlot> _buildChronologicalSlots() {
    ...
    for (final session in widget.historyList) {
      final key = DateFormat('yyyy-MM-dd').format(session.endTime);
      final existing = map[key];
      if (existing == null ||
          session.qualityScore > existing.score) {
        map[key] = _DaySlot(
          date: DateTime(session.endTime.year, session.endTime.month,
              session.endTime.day),
